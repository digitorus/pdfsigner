...
func main() {
	...
	doc := license.LicenseData{
		...
		Limits: []ratelimiter.Limit{
			...
			ratelimiter.Limit{Unlimited: false, MaxCount: 2, Interval: time.Minute},
			ratelimiter.Limit{Unlimited: false, MaxCount: 2, Interval: time.Hour},
			ratelimiter.Limit{Unlimited: false, MaxCount: 2, Interval: 24 * time.Hour},
			ratelimiter.Limit{Unlimited: false, MaxCount: 2, Interval: 720 * time.Hour},
		},
	}
	...
	str64, err := lic.ToB64String()
	if err != nil {
		log.Fatal(err)

	}
	log.Println("LicenseData:", str64)
